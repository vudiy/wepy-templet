...
export default class baseMixin extends wepy.mixin {
  ...
  // 判断是否是空 空字符串，空数组，空对象
  isEmpty(item) {
    if (this.isNull(item)) return true
    else if (this.isArray(item)) {
      if (item.length === 0) return true
    } else if (this.isObject(item)) {
      if (this.isEmptyObject(item)) return true
    }
    return false
  }
  /**
  ...
  isContain(arr, item) {
    return !!(this.isArray(arr) && arr.indexOf(item) >= 0)
  }
  ...
  getXmlInfo(className, callback) {
    this.getXml(className, rects => {
      if (typeof callback === 'function') {
        callback(rects.length > 0 ? rects[0] : null)
      }
    })
  }
